...
echo Running Pipseeker to make 10X formatted FASTQs
...
echo Concatenating threaded FASTQ files
#Concatenate all R1 and all R2 files back together
cat *R1* > barcodes_R1.fq.gz
cat *R2* > barcodes_R2.fq.gz
...
#Align cell barcodes
echo Aligning CBCs
bbduk.sh in=barcodes_R1.fq.gz out=cbc.fq ftr=15 qin=33
...
#Pull UMIs
echo Pulling UMIs
bbduk.sh in=barcodes_R1.fq.gz out=temp4.fq ftr=28 qin=33
